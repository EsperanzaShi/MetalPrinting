function [] = CLI_PLUS_ILT_Compiler_vk_2(PrintParameters, Layer_toolpaths, Part_ID)
        ...
        elseif strcmp('vs',PrintParameters.ScanningOrder(Part_Num,:)) %infills
            ...
            for i = 1:size(Layer_toolpaths(Layer_Num).Infill,2)
                ...
                elseif Layer_toolpaths(Layer_Num).Infill{i}.Type == 4 %hatches
                    TF = ischange(Layer_toolpaths(Layer_Num).Infill{i}.Power);
                    indx = [1, find(TF)', size(TF,1)];
                    TF2 = ischange(repelem(Layer_toolpaths(Layer_Num).Infill{i}.Power,4,1));
                    indx2 = [1, find(TF2)', size(TF2,1)];
                    for q = 1:size(indx,2)-1
                        fprintf(fileID,'%s\n', ['$$POWER/' num2str(round(Layer_toolpaths(Layer_Num).Infill{i}.Power(indx(q))))]);
                        fprintf(fileID,'%s\n', ['$$SPEED/' num2str(Layer_toolpaths(Layer_Num).Infill{i}.Speed(indx(q)))]);
                        fprintf(fileID,'%s\n', strcat(['$$HATCHES/' num2str(str2num(Part_ID)) ',' num2str(indx(q+1)-indx(q)) ','], strjoin(string(Layer_toolpaths(Layer_Num).Infill{i}.Points(indx2(q):indx2(q+1)).'),','))); % Hatches, ID, number of hatches, points (H1startx, H1starty, H1endx, H1endy,...Hnstartx, Hnstarty, Hnendx, Hnendy)
                    end

                    % count = 1;
                    % for q = 1:4:size(Layer_toolpaths(Layer_Num).Infill{i}.Points,1)
                    %     fprintf(fileID,'%s\n', ['$$POWER/' num2str(Layer_toolpaths(Layer_Num).Infill{i}.Power(count))]);
                    %     count = count +1;
                    %     fprintf(fileID,'%s\n', ['$$SPEED/' num2str(PrintParameters.Hatch_ScanSpeed)]);
                    %     fprintf(fileID,'%s\n', strcat(['$$HATCHES/' num2str(str2num(Part_ID)) ',1,'], strjoin(string(Layer_toolpaths(Layer_Num).Infill{i}.Points(q:q+3).'),','))); % Hatches, ID, number of hatches, points (H1startx, H1starty, H1endx, H1endy,...Hnstartx, Hnstarty, Hnendx, Hnendy)
                    % end
                end
            end
